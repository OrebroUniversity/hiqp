...
template <>
void ROSTopicSubscriber::topicCallback<tf2_msgs::msg::TFMessage>(
  ...
  for(int i=0; i<msg->transforms.size(); i++) {
    ...
    auto t = msg->transforms[i].transform;
    ...
    task_manager_->getGeometricPrimitiveMap()->setGeometricPrimitive(child_frame, "frame",
        parent_frame, true, 
        color, parameters);
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "added new frame %s",child_frame.c_str());
