    ...
    try {
      ...
      for (it; it!=stages_map_.end(); ++it) {
        ...
        try {
          ...
          std::cerr<<"In HQPSolver::solve(...): Gurobi exception with error code %d, and error message %s when trying to extract the solution variables.", e.getErrorCode(), e.getMessage().c_str());
      ...
      std::cerr<<"In HQPSolver::solve(...): Gurobi exception with error code %d, and error message %s.", e.getErrorCode(), e.getMessage().c_str());
      return false;
    }
    return true;
  }
