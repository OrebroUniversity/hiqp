...
bool GurobiSolver::solve(std::vector<double>& solution) {
  ...
  try {
    ...
    for (it; it != stages_map_.end(); ++it) {
      ...
      unsigned int s_acc_dim = b_.rows();  // accumulated dimensions of all the
                                           // previously solved stages

      // append the new signs, jacobian matrix and task velocity vector to the
      // previous ones
      ...
      w_.tail(s_dim).setZero();  // set zero for now - will be filled with the
                                 // solution later

      // at each iteration, variables are x (the joint velocities) + the slack
      // variables
      double* lb_x = new double[x_dim];
      std::fill_n(lb_x, x_dim, -GRB_INFINITY);
      double* ub_x = new double[x_dim];
      std::fill_n(ub_x, x_dim, GRB_INFINITY);
      GRBVar* x = model.addVars(lb_x, ub_x, NULL, NULL, NULL, x_dim);

      double* lb_w = new double[s_dim];
      std::fill_n(lb_w, s_dim, -GRB_INFINITY);
      double* ub_w = new double[s_dim];
      std::fill_n(ub_w, s_dim, GRB_INFINITY);
      GRBVar* w = model.addVars(lb_w, ub_w, NULL, NULL, NULL,
                                s_dim);  // slack variables
      model.update();
      ...
      for (unsigned int i = 0; i < s_dim; i++) {
        ...
        if (s_count == 1)
          lhsides[s_acc_dim + i] -= w[i] * 0.0;  // force the slack variables to
                                                 // be zero in the highest
                                                 // stage;
        else
      ...
      // add constraints
      GRBConstr* constrs = model.addConstrs(lhsides, &senses_[0], rhsides, NULL,
                                            s_acc_dim + s_dim);
      ...
      if (status != GRB_OPTIMAL) {
        if (status == GRB_TIME_LIMIT)
          ROS_WARN(
              "Stage solving runtime %f sec exceeds the set time limit of %f "
              "sec.",
              runtime, TIME_LIMIT);
        else
          ROS_ERROR(
              "In HQPSolver::solve(...): No optimal solution found for stage "
              "%d. Status is %d.",
              s_count, status);
      ...
      try {
        ...
        std::cerr
            << "In HQPSolver::solve(...): Gurobi exception with error code"
            << e.getErrorCode() << ", and error message "
            << e.getMessage().c_str()
            << " when trying to extract the solution variables." << std::endl;
    ...
    std::cerr << "In HQPSolver::solve(...): Gurobi exception with error code "
              << e.getErrorCode() << ", and error message "
