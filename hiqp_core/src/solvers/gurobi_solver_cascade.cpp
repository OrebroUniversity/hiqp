  ...
  for (auto&& kv : stages_map_) {
    ...
    try {
      ...
      ROS_ERROR_THROTTLE(10,
                                 "In GurobiSolverCascade::QPProblem::getSolution : "
...
void GurobiSolverCascade::QPProblem::update(GRBEnv &env_, HQPConstraints& hqp_constraints) {
  ...
  bool GurobiSolverCascade::QPProblem::solve() {
	...
	//ROS_WARN("In GurobiSolverCascade::QPProblem::solve(...): Stage solving runtime %f sec exceeds the set time limit of %f sec.",
//		 runtime, TIME_LIMIT);
      }
      else if (status == GRB_SUBOPTIMAL){
	ROS_WARN("In GurobiSolverCascade::QPProblem::solve(...): Only suboptimal QP solution found.");
      }
      else if (status == GRB_NUMERIC){
	ROS_WARN("In GurobiSolverCascade::QPProblem::solve(...): QP optimization was terminated due to unrecoverable numerical difficulties.");
      }
      else if (status == GRB_ITERATION_LIMIT){
	ROS_WARN("In GurobiSolverCascade::QPProblem::solve(...): QP optimization was terminated because iteration limit was reached.");
      }       
      else{
	ROS_ERROR("In GurobiSolverCascade::QPProblem::solve(...): No optimal solution found for stage with "
