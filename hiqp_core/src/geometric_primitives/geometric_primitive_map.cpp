...
int GeometricPrimitiveMap::setGeometricPrimitive(
  ...
  if (type.compare("point") == 0) {
    auto primitive =
        std::make_shared<GeometricPoint>(name, frame_id, visible, color);
    ...
    auto primitive =
        std::make_shared<GeometricLine>(name, frame_id, visible, color);
    if (primitive->init(parameters) == 0) {
    ...
    auto primitive =
        std::make_shared<GeometricPlane>(name, frame_id, visible, color);
    if (primitive->init(parameters) == 0) {
    ...
    auto primitive =
        std::make_shared<GeometricBox>(name, frame_id, visible, color);
    if (primitive->init(parameters) == 0) {
    ...
    auto primitive =
        std::make_shared<GeometricCylinder>(name, frame_id, visible, color);
    if (primitive->init(parameters) == 0) {
    ...
    auto primitive =
        std::make_shared<GeometricSphere>(name, frame_id, visible, color);
    if (primitive->init(parameters) == 0) {
    ...
    auto primitive =
        std::make_shared<GeometricFrame>(name, frame_id, visible, color);
    if (primitive->init(parameters) == 0) {
...
int GeometricPrimitiveMap::removeGeometricPrimitive(std::string name) {
  if (std::find(all_primitive_names_.begin(), all_primitive_names_.end(),
                name) == all_primitive_names_.end()) {
    printHiqpWarning(
        "While trying to remove primitive with name '" + name +
  ...
  if (dependencies.size() > 0) {
    printHiqpWarning(
        "Geometric primitive '" + name + "' has the following dependencies: " +
        dependencies +
  ...
  all_primitive_names_.erase(std::remove(all_primitive_names_.begin(),
...
void GeometricPrimitiveMap::addDependencyToPrimitive(
    const std::string& primitive_name, const std::string& dependency_name) {
  ...
  if (it == dependency_map_.end()) {
    printHiqpWarning("Trying to add dependency to geometric primitive '" +
                     primitive_name +
                     "'. No such primitive found. No dependency was added!");
  }

  std::vector<std::string>::iterator it2 =
      std::find(it->second.begin(), it->second.end(), dependency_name);

  if (it2 == it->second.end()) it->second.push_back(dependency_name);
}
...
void GeometricPrimitiveMap::removeDependency(
  ...
  for (auto&& kv : dependency_map_) {
    kv.second.erase(
        std::remove(kv.second.begin(), kv.second.end(), dependency_name),
        kv.second.end());
  }
...
void GeometricPrimitiveMap::acceptVisitor(GeometricPrimitiveVisitor& visitor,
  ...
  if (primitive_name.compare("") == 0) {
    ...
    if (std::find(all_primitive_names_.begin(), all_primitive_names_.end(),
                  primitive_name) != all_primitive_names_.end()) {
      {
...
std::vector<PrimitiveInfo> GeometricPrimitiveMap::getAllPrimitiveInfo() {
  ...
  for (auto it : point_map_) {
    all_primitive_info.push_back(
        PrimitiveInfo(it.second->getName(), "point", it.second->getFrameId(),
                      it.second->isVisible(), it.second->getColor(),
  ...
  for (auto it : line_map_) {
    all_primitive_info.push_back(
        PrimitiveInfo(it.second->getName(), "line", it.second->getFrameId(),
                      it.second->isVisible(), it.second->getColor(),
                      it.second->getParameters()));
  }
  ...
  for (auto it : plane_map_) {
    all_primitive_info.push_back(
        PrimitiveInfo(it.second->getName(), "plane", it.second->getFrameId(),
                      it.second->isVisible(), it.second->getColor(),
                      it.second->getParameters()));
  }
  ...
  for (auto it : box_map_) {
    all_primitive_info.push_back(
        PrimitiveInfo(it.second->getName(), "box", it.second->getFrameId(),
                      it.second->isVisible(), it.second->getColor(),
                      it.second->getParameters()));
  }
  ...
  for (auto it : cylinder_map_) {
    all_primitive_info.push_back(
        PrimitiveInfo(it.second->getName(), "cylinder", it.second->getFrameId(),
                      it.second->isVisible(), it.second->getColor(),
                      it.second->getParameters()));
  }
  ...
  for (auto it : sphere_map_) {
    all_primitive_info.push_back(
        PrimitiveInfo(it.second->getName(), "sphere", it.second->getFrameId(),
                      it.second->isVisible(), it.second->getColor(),
                      it.second->getParameters()));
  }
  ...
  for (auto it : frame_map_) {
    all_primitive_info.push_back(
        PrimitiveInfo(it.second->getName(), "frame", it.second->getFrameId(),
                      it.second->isVisible(), it.second->getColor(),
                      it.second->getParameters()));
  }
...
template <>
std::shared_ptr<GeometricPoint> GeometricPrimitiveMap::getGeometricPrimitive<
...
template <>
std::shared_ptr<GeometricLine> GeometricPrimitiveMap::getGeometricPrimitive<
...
template <>
std::shared_ptr<GeometricPlane> GeometricPrimitiveMap::getGeometricPrimitive<
...
template <>
std::shared_ptr<GeometricBox> GeometricPrimitiveMap::getGeometricPrimitive<
...
template <>
std::shared_ptr<GeometricCylinder> GeometricPrimitiveMap::getGeometricPrimitive<
...
template <>
std::shared_ptr<GeometricSphere> GeometricPrimitiveMap::getGeometricPrimitive<
...
template <>
std::shared_ptr<GeometricFrame> GeometricPrimitiveMap::getGeometricPrimitive<
...
void GeometricPrimitiveMap::updateGeometricPrimitive<GeometricPoint>(
  ...
  PointMap::iterator it = point_map_.find(name);
  if (it == point_map_.end()) {
    printHiqpWarning("Couldn't update GeometricPoint with name '" + name +
...
void GeometricPrimitiveMap::updateGeometricPrimitive<GeometricLine>(
  ...
  LineMap::iterator it = line_map_.find(name);
  if (it == line_map_.end()) {
    printHiqpWarning("Couldn't update GeometricLine with name '" + name +
...
void GeometricPrimitiveMap::updateGeometricPrimitive<GeometricPlane>(
  ...
  PlaneMap::iterator it = plane_map_.find(name);
  if (it == plane_map_.end()) {
    printHiqpWarning("Couldn't update GeometricPlane with name '" + name +
...
void GeometricPrimitiveMap::updateGeometricPrimitive<GeometricBox>(
  ...
  BoxMap::iterator it = box_map_.find(name);
  if (it == box_map_.end()) {
    printHiqpWarning("Couldn't update GeometricBox with name '" + name +
...
void GeometricPrimitiveMap::updateGeometricPrimitive<GeometricCylinder>(
  ...
  CylinderMap::iterator it = cylinder_map_.find(name);
  if (it == cylinder_map_.end()) {
    printHiqpWarning("Couldn't update GeometricCylinder with name '" + name +
                     "'. The cylinder was not found!");
    return;
  }
  it->second->init(parameters);
}
...
void GeometricPrimitiveMap::updateGeometricPrimitive<GeometricSphere>(
  ...
  SphereMap::iterator it = sphere_map_.find(name);
  if (it == sphere_map_.end()) {
    printHiqpWarning("Couldn't update GeometricSphere with name '" + name +
                     "'. The sphere was not found!");
    return;
  }
  it->second->init(parameters);
}
...
std::string GeometricPrimitiveMap::getDependenciesAsString(std::string name) {
