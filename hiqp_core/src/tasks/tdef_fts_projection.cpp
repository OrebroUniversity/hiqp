...
template <>
int TDefFTSProjection<GeometricPoint, GeometricPoint>::projectForces(
    std::shared_ptr<GeometricPoint> point1,
    std::shared_ptr<GeometricPoint> point2, const RobotStatePtr robot_state) {

  // KDL::Vector p1__ = pose_a_.M * point1->getPointKDL(); //point 1 from link
  // origin to ee expressed in the world frame
  // KDL::Vector p1 = pose_a_.p + p1__;  //absolute ee point 1 expressed in the
  // world frame

  // KDL::Vector p2__ = pose_b_.M * point2->getPointKDL(); //point 2 from link
  // origin to ee expressed in the world frame
  // KDL::Vector p2 = pose_b_.p + p2__; //absolute ee point 2 expressed in the
  // world frame
  ...
  // changeJacDotRefPoint(jacobian_a_,jacobian_dot_a_,robot_state->kdl_jnt_array_vel_,
  // p1__, J_dot_p1p2);
  // changeJacDotRefPoint(jacobian_b_,jacobian_dot_b_,robot_state->kdl_jnt_array_vel_,
  // p2__, J_dot_p2);
  ...
  // // double eps=1e-5;
  // // J_=Eigen::Map<Eigen::Matrix<double,1,3>
  // >(d.data)/(e_(0)+eps)*J_p1p2.data.topRows<3>();
  // // J_dot_=Eigen::Map<Eigen::Matrix<double,1,3>
  // >(d.data)/(e_(0)+eps)*J_dot_p1p2.data.topRows<3>()+(d_dot.transpose()/(e_(0)+eps)
  // - (Eigen::Map<Eigen::Matrix<double,1,3>
  // >(d.data)*d_dot)*Eigen::Map<Eigen::Matrix<double,1,3>
  // >(d.data)/pow(pow(e_(0)+eps,2),1.5))*J_p1p2.data.topRows<3>();
  // // e_dot_=J_*qdot;
  //       // END VARIANT 1 ==============================================

  //       // VARIANT 2: e=d^T*d ==============================================
  // e_(0)=pow(d.Norm(),2);
  // J_=2*Eigen::Map<Eigen::Matrix<double,1,3>
  // >(d.data)*J_p1p2.data.topRows<3>();
  // e_dot_=J_*qdot;
  // J_dot_=2*(d_dot.transpose()*J_p1p2.data.topRows<3>()+Eigen::Map<Eigen::Matrix<double,1,3>
  // >(d.data)*J_dot_p1p2.data.topRows<3>());
  //             // END VARIANT 2 ==============================================
  // DEBUG =========================================================
  // std::cerr<<"q_in:
  // "<<robot_state->kdl_jnt_array_vel_.q.data.transpose()<<std::endl;
...
template <>
int TDefFTSProjection<GeometricPoint, GeometricPlane>::projectForces(
    std::shared_ptr<GeometricPoint> point,
    std::shared_ptr<GeometricPlane> plane, const RobotStatePtr robot_state) {

  hiqp::SensorHandleInfo sensor = robot_state->sensor_handle_info_[sensor_id_];
  Eigen::Vector3d n = Eigen::Map<Eigen::Vector3d>(
      (pose_b_.M * plane->getNormalKDL())
          .data); // plane normal expressed in the world frame

  Eigen::Vector3d force = Eigen::Map<Eigen::Vector3d>((pose_fts_.M * KDL::Vector(sensor.force_(0), sensor.force_(1), sensor.force_(2))).data); // measured force expressed in the world frame
  
  f_ = force.transpose() * n;

  // DEBUG =========================================================
  // std::cerr << "f_: " << f_ << std::endl;
  // DEBUG END =====================================================
...
//     //   KDL::Rotation S =
//     //       box->getScalingKDL();  // from the world frame to a unit-box
//     frame
...
//     //   KDL::Vector x =
//     //       S * R.Inverse() *
//     //       (p - c);  // vector from c to p in axis-aligned unit-box
//     coordinates

//     //   double f = absMax({x.x(), x.y(), x.z()});
//     //   double lambda = 1 / (2 * f);

//     //   KDL::Vector x_prim = lambda * x;  // the projected point p' on the
//     box in
...
//     // template <>
//     // int TDefFTSProjection<GeometricPoint,
//     GeometricCylinder>::projectForces(std::shared_ptr<GeometricPoint> point,
//     // 									    std::shared_ptr<GeometricCylinder>
//     cylinder,
//     // 									    const RobotStatePtr robot_state)
//     {
//     //   KDL::Vector p2__ = pose_a_.M * point->getPointKDL(); //ee point
//     expressed in the world frame
//     //   KDL::Vector p2 = pose_a_.p + p2__; //absolute ee-point expressed in
//     the world frame
//     //   KDL::Vector v__ = pose_b_.M * cylinder->getDirectionKDL();
//     //cylinder axis expressed in the world frame
//     //   KDL::Vector k__ = pose_b_.M * cylinder->getOffsetKDL();// point on
//     the cylinder axis expressed in the wrold frame
//     //   KDL::Vector k = pose_b_.p + k__;//absolute cylinder axis point
//     expressed in the world frame

//     //   Eigen::Vector3d v = Eigen::Map<Eigen::Matrix<double,3,1>
//     >(v__.data);//convenience redefinition of v as an Eigen vector
//     //   Eigen::Vector3d p2k=Eigen::Map<Eigen::Matrix<double,3,1> >(p2.data)
//     - Eigen::Map<Eigen::Matrix<double,3,1> >(k.data); //convenience
//     definition of the difference vector between p2 and k
//     //   Eigen::Vector3d d=v.transpose().dot(p2k)*v-p2k; //vector pointing
//     from ee-point p2 to the closest point on the cylinder axis
...
//     //   Eigen::VectorXd qdot=robot_state->kdl_jnt_array_vel_.qdot.data;
//     //   Eigen::MatrixXd J__ =
//     J_k.data.topRows<3>()+v*p2k.transpose()*J_v.data.topRows<3>()-J_p2.data.topRows<3>()+v.transpose().dot(p2k)*J_v.data.topRows<3>()+v*v.transpose()*(J_p2.data.topRows<3>()
//     - J_k.data.topRows<3>());
...
//     // template <>
//     // int TDefFTSProjection<GeometricPoint,
//     GeometricSphere>::projectForces(std::shared_ptr<GeometricPoint> point,
//     // 									  std::shared_ptr<GeometricSphere>
//     sphere,
//     // 									  const RobotStatePtr robot_state)
//     {

//     //  KDL::Vector p1__ = pose_a_.M * point->getPointKDL(); //point 1 from
//     link origin to ee expressed in the world frame
//     //   KDL::Vector p1 = pose_a_.p + p1__;  //absolute ee point 1 expressed
//     in the world frame

//     //   KDL::Vector p2__ = pose_b_.M * sphere->getCenterKDL(); //point 2
//     from link origin to ee expressed in the world frame
//     //   KDL::Vector p2 = pose_b_.p + p2__; //absolute ee point 2 expressed
//     in the world frame

//     //   KDL::Vector d = p1 - p2; //distance vector expressed in the world
//     frame
...
//     //   // s are the parameter values for line1, line2 and line3 indicating
//     the points
//     //   // of intersection
...
//     //   KDL::Vector d3 =
//     //       v1 * s.data[0] +
//     //       d1;  // the point in world coordinates that lies on line1 and
//     line3
//     //   KDL::Vector d3_proj =
//     //       v2 * s.data[1] +
//     //       d2;  // the point in world coordinates that lies on line2 and
//     line3
...
//     // template <>
//     // int TDefFTSProjection<GeometricSphere,
//     GeometricPlane>::projectForces(std::shared_ptr<GeometricSphere> sphere,
//     // 									  std::shared_ptr<GeometricPlane>
//     plane,
//     // 									  const RobotStatePtr robot_state)
//     {

//     //   KDL::Vector p1__ = pose_a_.M * sphere->getCenterKDL(); //point 1
//     from link origin to sphere center expressed in the world frame
//     //   KDL::Vector p1 = pose_a_.p + p1__; //absolute ee point 1 expressed
//     in the world frame

//     //   KDL::Vector n = pose_b_.M * plane->getNormalKDL();  //plane normal
//     expressed in the world frame
//     //   KDL::Vector k__ = plane->getOffset() * n; //point on the plane
//     expressed in the world frame
//     //   KDL::Vector v__ = k__ + n; //normal tip expressed in the world frame
//     //   KDL::Vector k = k__ + pose_b_.p; // absolute point on the plane
//     expressed in the world frame
...
//     //   changeJacDotRefPoint(jacobian_b_, jacobian_dot_b_,
//     robot_state->kdl_jnt_array_vel_, v__, J_dot_vk);
//     //   changeJacDotRefPoint(jacobian_b_, jacobian_dot_b_,
//     robot_state->kdl_jnt_array_vel_, k__, J_dot_k);
//     //   J_dot_vk.data = J_dot_vk.data - J_dot_k.data;

//     //   changeJacDotRefPoint(jacobian_a_, jacobian_dot_a_,
//     robot_state->kdl_jnt_array_vel_, p1__, J_dot_p1k);
//     //   J_dot_p1k.data = J_dot_p1k.data - J_dot_k.data;

//     //   e_(0)=dot(n,p1)-b-sphere->getRadius();
//     //   J_=Eigen::Map<Eigen::Matrix<double,1,3>
//     >((p1-k).data)*J_vk.data.topRows<3>()+Eigen::Map<Eigen::Matrix<double,1,3>
//     >(n.data)*J_p1k.data.topRows<3>();
//     //   Eigen::VectorXd qdot=robot_state->kdl_jnt_array_vel_.qdot.data;
//     //   e_dot_=J_*qdot;
//     //   J_dot_=Eigen::Map<Eigen::Matrix<double,1,3>
//     >((p1-k).data)*J_dot_vk.data.topRows<3>()+Eigen::Map<Eigen::Matrix<double,1,3>
//     >(n.data)*J_dot_p1k.data.topRows<3>()+(J_p1k.data.topRows<3>()*qdot).transpose()*J_vk.data.topRows<3>()+(J_vk.data.topRows<3>()*qdot).transpose()*J_p1k.data.topRows<3>();
//     //   return 0;
//     // }

//     // template <>
//     // int TDefFTSProjection<GeometricSphere,
//     GeometricSphere>::projectForces(std::shared_ptr<GeometricSphere> sphere1,
//     // 									   std::shared_ptr<GeometricSphere>
//     sphere2,
//     // 									   const RobotStatePtr robot_state)
//     {
//     //   KDL::Vector p1__ = pose_a_.M * sphere1->getCenterKDL(); //point 1
//     from link origin to the sphere center expressed in the world frame
//     //   KDL::Vector p1 = pose_a_.p + p1__;  //absolute ee point 1 expressed
//     in the world frame

//     //   KDL::Vector p2__ = pose_b_.M * sphere2->getCenterKDL(); //point 2
//     from link origin to the sphere center expressed in the world frame
//     //   KDL::Vector p2 = pose_b_.p + p2__; //absolute ee point 2 expressed
//     in the world frame
...
//    //  template <>
//    //  int TDefFTSProjection<GeometricFrame,
//    GeometricFrame>::projectForces(std::shared_ptr<GeometricFrame> frame1,
//    // 									 std::shared_ptr<GeometricFrame>
//    frame2,
//    // 									 const RobotStatePtr robot_state)
//    {
//    // KDL::Vector p1__ = pose_a_.M * frame1->getCenterKDL(); //point 1 from
//    link origin to the frame center expressed in the world frame
//    //    KDL::Vector p1 = pose_a_.p + p1__;  //absolute ee point 1 expressed
//    in the world frame

//    //    KDL::Vector p2__ = pose_b_.M * frame2->getCenterKDL(); //point 2
//    from link origin to the sframe center expressed in the world frame
//    //    KDL::Vector p2 = pose_b_.p + p2__; //absolute ee point 2 expressed
//    in the world frame
