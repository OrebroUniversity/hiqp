...
class GeometricBox : public GeometricPrimitive {
 public:
  GeometricBox(const std::string& name, const std::string& frame_id,
   ...
   * center of the box.</li>
   *                      <li>Indices 3-5 (required) defines the dimensions of
   * the box.</li>
   *                      <li>Indices 6-8 (optional) defines euler angles of the
   * orientation of the box (xyz).</li>
   *                      <li>Indices 6-9 (optional) defines a quaternion for
   * the orientation of the box.</li>
   *                      </ol>
  ...
  int init(const std::vector<double>& parameters) {
    ...
    scaling_matrix_eigen_ << 1 / kdl_dim_(0), 0, 0, 0, 1 / kdl_dim_(1), 0, 0, 0,
        1 / kdl_dim_(2);

    scaling_matrix_kdl_ = KDL::Rotation(
        1 / kdl_dim_(0), 0, 0, 0, 1 / kdl_dim_(1), 0, 0, 0, 1 / kdl_dim_(2));

    // inversion of KDL::Rotate does not work the same as inverting a matrix
    scaling_matrix_inverted_kdl_ =
        KDL::Rotation(kdl_dim_(0), 0, 0, 0, kdl_dim_(1), 0, 0, 0, kdl_dim_(2));
  ...
  inline const KDL::Rotation& getScalingInvertedKDL() {
    return scaling_matrix_inverted_kdl_;
  }

  inline const Eigen::Matrix3d& getScalingEigen() {
    return scaling_matrix_eigen_;
  }

  /// \brief Returns the rotation matrix that transforms from global world
  /// coordinates to local box coordinates
  inline const KDL::Rotation& getRotationKDL() { return rotation_kdl_; }

  /// \brief Returns the quaternion that transforms from global world
  /// coordinates to local box coordinates
  inline const Eigen::Quaternion<double>& getQuaternionEigen() { return q_; }

  /// \brief Gets the quaternion components that transform from global world
  /// coordinates to local box coordinates
