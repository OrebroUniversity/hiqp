...
/*! \brief A Geometric Primitive Visitor that sends the primitive it visits to a
 * visualizer.
...
class GeometricPrimitiveVisualizer : public GeometricPrimitiveVisitor {
 public:
   ...
   * If action=1, removeAllVisitedPrimitives has to be called after finishing
   * visiting to perform the actual removal.
   *  \param action : 0 for adding/updating the visited geometric primitive to
   * the visualizer, 1 for removing it. */
 ...
 private:
  GeometricPrimitiveVisualizer(const GeometricPrimitiveVisualizer& other) =
      delete;
  GeometricPrimitiveVisualizer(GeometricPrimitiveVisualizer&& other) = delete;
  GeometricPrimitiveVisualizer& operator=(
      const GeometricPrimitiveVisualizer& other) = delete;
  GeometricPrimitiveVisualizer& operator=(
      GeometricPrimitiveVisualizer&& other) noexcept = delete;
...
template <typename Primitive>
void GeometricPrimitiveVisualizer::visit__(
    std::shared_ptr<Primitive> primitive) {
  int id = primitive->getVisualId();
  switch (action_) {
    case 0:
      if (id < 0)
        primitive->setVisualId(visualizer_->add(primitive));
      else
        visualizer_->update(id, primitive);
      break;
    case 1:
...
template <>
void GeometricPrimitiveVisualizer::visit__<GeometricFrame>(
    std::shared_ptr<GeometricFrame> primitive) {
  int id = primitive->getVisualId();
  switch (action_) {
    case 0:
      if (id < 0)
