...
/*! \brief Holds information on what q_nr (KDL) is associated with what joint
...
struct RobotState {
  ...
  /// \brief Returns the total number of joints (including read-only joint
  /// resources)
  inline unsigned int getNumJoints() const { return kdl_tree_.getNrOfJoints(); }

  /// \brief Returns the number of controllable joints (writable joint
  /// resources)
